...
type (
	userRoleModel interface {
		...
		FindOneByUserIdRoleId(ctx context.Context, userId uint64, roleId uint64) (*UserRole, error)
	...
	UserRole struct {
		...
		RoleId    uint64    `db:"role_id"`    // 角色 ID
		CreatedAt time.Time `db:"created_at"` // 分配角色的时间
	}
)
...
func (m *defaultUserRoleModel) FindOneByUserIdRoleId(ctx context.Context, userId uint64, roleId uint64) (*UserRole, error) {
